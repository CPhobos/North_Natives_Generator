...
native_name_pattern = re.compile(r"(\w+)\s*\(")
...
def get_formatted_args(input_args: list) -> str:
	return remove_py_keywords(re.sub(arg_pattern,  "", input_args[i]))
...
def get_return_types(src: str) -> list:
...
def get_native_args(src: str) -> list:
	match = re.findall("\w+\((.*?)\)", src)
	if(len(match) != 0):
		return match
	return [""]

def format_native_names(src: str) -> str:
	match = re.search("0x[A-Za-z0-9]+", src) 
	if(match):
		return re.sub("0x[A-Za-z0-9]+", "_{}".format(match.group(0)), src)
	else:
		return src 

def get_native_names(src: str, _format: str) -> list:
	if(_format == "fivem"):
		match = re.findall(native_name_pattern, fivem_format(src))
	elif(_format == "snake_lower"):
		match = re.findall(native_name_pattern, src.lower())
	elif(_format == "snake_cap"):
		match = re.findall(native_name_pattern, src)
	if(match):
		return match
	return [""]
...
if __name__ == "__main__":
	...
	with open("natives.py", "w") as natives_file:
		natives_file.writelines('# Generated By Phobos {}\n'.format(now.strftime("%d/%m/%Y %H:%M:%S")))
		natives_file.write('# https://github.com/CorPhobos/North_Natives_Generator\n')
		for i in range(len(native_hashes)):
			name = format_native_names(native_names[i])
			args = get_formatted_args(native_args)
